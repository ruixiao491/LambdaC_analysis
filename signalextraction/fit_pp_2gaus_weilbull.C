...
TFile *f = new TFile("/home/xiao147/private/newchannel_lambda_CtoproduceDntuple/CMSSW_7_5_8_patch3/src/Bfinder/Bfinder/Dntuple/hist_pp_6_8_TMVA_latest_whole.root");
...
//TF1 *f3=new TF1("f3","0.007*[0]*([3]*TMath::Gaus(x,[1],[2])/(sqrt(2*3.14159)*[2])+(1-[3])*TMath::Gaus(x,[1],[4])/(sqrt(2*3.14159)*[4]))+[5]+[6]*(1-exp(-x*[7]))",2.1,2.45);
f3->SetParNames("Strength","Mean1","Sigma1","pro1","Sigma2","back1","back2");
Double_t par[10];
...
/*
h1->Fit("f3","","",2.25,2.3); // 10-20 pp 2 gaus 1 step
h1->Fit("f3","","",2.21,2.38);//10-20 pp 2 gaus 2 step
h1->Fit("f3","","",2.2,2.4);//10-20 pp 2 gaus 3 step
//h1->Fit("f3","","",2.2,2.37);//10-20 pp 2 gaus 4 step
//h1->Fit("f3","","",2.2,2.4);//10-20 pp 2 gaus 5 step 
*/

h1->Fit("f3","","",2.258,2.3);// 6-8 pp 2 gaus 1 step
h1->Fit("f3","","",2.22,2.38);//6-8 pp 2 gaus 2 step
h1->Fit("f3","","",2.1,2.38);//6-8 pp 2 gaus 3 step
//h1->Fit("f3","","",2.18,2.425);//6-8 pp 2 gaus 4 step

/*
h1->Fit("f3","","",2.258,2.31);//5-6 pp 2 gaus 1 step
h1->Fit("f3","","",2.22,2.38);//5-6 pp 2 gaus 2 step
h1->Fit("f3","","",2.1,2.45);//5-6 pp 2 gaus 3 step
//h1->Fit("f3","","",2.18,2.4);//5-6 pp 2 gaus 4 step
//h1->Fit("f3","","",2.2,2.4);// 5-6 pp 2 gaus 5 step
*/
/*
h1->Fit("f3","","",2.25,2.3);//8-10 pp 2 gaus 1 step
h1->Fit("f3","","",2.18,2.45);//8-10 pp 2 gaus 2 step
h1->Fit("f3","","",2.1,2.45);//8-10 pp 2 gaus 3 step
//h1->Fit("f3","","",2.1,2.41);//8-10 pp 2 gaus 4 step
//h1->Fit("f3","","",2.2,2.4);//8-10 pp 2 gaus 5 step
*/

f3->GetParameters(par);
f1->SetParameter(0,par[5]);
f1->SetParameter(1,par[6]);
f1->SetParameter(2,par[7]);
f1->SetParameter(3,par[8]);
f1->SetParameter(4,par[9]);
...
TLatex* tex;
tex = new TLatex(0.20,0.73,"6GeV<P_{T}<8GeV");
tex->SetNDC();
tex->SetTextFont(42);
...
c1->SaveAs("6_8_pplatestTMVA_whole_2gaus_weibull.gif");
//h1->Write();
//output->Close();

TFile* result = new TFile("6_8_pp_TMVAcuts_2gaus_weibull.root", "RECREATE");
h1->Write();
f1->Write();
//f2->Write();
f3->Write();
