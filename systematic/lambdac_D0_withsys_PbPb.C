...
void lambdac_D0_withsys_PbPb(){
    ...
    double x[2]={9,15};
	double py[2]={ h_ratio_pp->GetBinContent(3), h_ratio_pp->GetBinContent(4)};
	double zero[2] ={1,5};
	double ey_stat[2]={h_ratio_pp->GetBinError(3), h_ratio_pp->GetBinError(4)};
	double ey_sys[2]={0, hsys_sum->GetBinContent(4)/100*h_ratio_pp->GetBinContent(4)};
